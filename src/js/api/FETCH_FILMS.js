...
export default {
  allFilms (page) {
    ...
    return fetch(baseUrl + params + `&api_key=${apiKey}` + `&page=${page}`)
      .then(response => response.json());
  },
  filmInfo (id) {
    const params = `&language=ru-RU`;

    return fetch(baseUrl + `movie/${id}` + `?api_key=${apiKey}` + params)
      .then(response => response.json());
  },
  searchFilms (query, page) {
    const params = `search/movie?language=ru-RU`;

    return fetch(baseUrl + params + `&api_key=${apiKey}` + `&query=${query}` + `&page=${page}`)
      .then(response => response.json());
  },
};
